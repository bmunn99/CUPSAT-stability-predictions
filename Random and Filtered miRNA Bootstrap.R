...
ECM_mirnas <- read.csv("/Users/bmunn99/Desktop/P2 Pathways/Neuron Projection Development/unique_mirnas_unfiltered.csv", sep = ',', header = TRUE)
...
random_bootstrap_top10_avg <- function(data, n) {
  replicate(n, {
    ...
    genes <- sample(unique(data$gene_name), 14)
...
filtered_bootstrap_top10_avg <- function(data, n) {
  ...
  selected_miRNAs <- c('hsa-miR-124-3p','hsa-miR-107','hsa-miR-129-2-3p','hsa-let-7b-5p','hsa-miR-1-3p',
                       'hsa-miR-155-5p','hsa-miR-16-5p','hsa-miR-182-5p','hsa-miR-101-3p','hsa-miR-103a-3p')
...
p2 <- ggplot(all_data, aes(x = Source, y = total_occurrences)) + 
  ...
  stat_summary(fun.data = function(y) data.frame(y = mean(y), size = 5, label = round(mean(y), 2)), geom = "text", vjust = -1.5) +
  theme_minimal() +
  ...
  geom_signif(comparisons = list(c("Random_P2", "ECM_mirnas")), 
              ...
              textsize = 3.5, 
              vjust = -1.5) +
  scale_x_discrete(labels = c('ECM_mirnas' = 'Neuron Projection miRNAs', 'Random_P2' = 'Random P2')) +
  annotate("text", x = n_P2$Source, y = 22, label = paste("n =", n_P2$n), color = "black")
p2
ggsave("/Users/bmunn99/Desktop/P2 Pathways/Neuron Projection Development/Comparison of ECM miRNAs vs. Random P2.png", p2, width = 10, height = 10, dpi = 300)
...
p2 <- ggplot(all_data, aes(x = Source, y = total_occurrences)) + 
  ...
  geom_signif(comparisons = list(c("new_Filtered_P2", "Filtered_P2")), 
              ...
              textsize = 3.5, 
              vjust = -1.5) +
  scale_x_discrete(labels = c('Filtered_P2' = 'Neuron Projection miRNAs in P2', 'new_Filtered_P2' = 'Random miRNAs in P2')) +
  annotate("text", x = n_P2$Source, y = 16, label = paste("n =", n_P2$n), color = "black")
p2
ggsave("/Users/bmunn99/Desktop/P2 Pathways/Neuron Projection Organization/Top ECM miRNAs in P1 vs Top Random miRNAs in P2.png", p2, width = 10, height = 10, dpi = 300)
